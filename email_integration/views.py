...
def send_transcribed_email(request):
    if request.method == 'POST':
        ...
        email_content = report_template(fields["subject"],fields["reporter_name"],edited_text)
        ...
        response = send_email(email_content, fields['subject'], recipient_email, fields['reporter_name'])
        return JsonResponse({'status': 'Email sent successfully!', 'response': response})
