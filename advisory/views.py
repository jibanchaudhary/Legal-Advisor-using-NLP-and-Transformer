...
def advisory_query(request):
    if request.method == 'POST':
        ...
        if not query:
            return JsonResponse({'error': 'Oops! It looks like you forgot to enter a query. Please provide one and try again.'})
        ...
        # labeling the query from generate_label funciton from legal_bert
            ...
            if match_crime_type(label):
                ...
                incident_embeddings = semantic_model.encode(incident_corpus, convert_to_numpy=True)
                policy_embeddings = semantic_model.encode(policy_corpus, convert_to_numpy=True)
                ...
                incident_results = retrieve_top_k(query, incident_index, incident_embeddings, incident_corpus)
                policy_results = retrieve_top_k(query, policy_index, policy_embeddings, policy_corpus)
                ...
                for doc, score in incident_results:
                    if score < thresold:
                        for incident in incidents:
                            if incident["title"].lower() in doc or incident["description"].lower() in doc:
                ...
                for doc, score in policy_results:
                    if score < thresold:
                        for policy in policies:
                            if policy["title"].lower() in doc or policy["description"].lower() in doc:
                                best_policy = policy
                                break  # Exit the inner loop immediately when a match is found
                    if best_policy:
                        break  # Exit the outer loop immediately when a match is found
                ...
                bert_recommendations=generate_recommendations_bert(category,crime_recommendations)
                bert_recommendations_formatted = format_recommendations(bert_recommendations)
                context = {
                    'incident': {
                        'title': best_incident['title'] if best_incident else "No incident found",
                            'description': summary_incident if best_incident else "No description available"
                    },
                    'policy': {
                        'title': best_policy['title'] if best_policy else "No policy found",
                            'description': summary_policy if best_policy else "No description available"  
                    },
